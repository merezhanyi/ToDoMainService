...
public class AccountController {
	...
	private UserDetails getUserDetailsWithEncodedPassword(Account account) {
		...
		var userPR = userP.roles(account.getRole());
	...
	public ResponseEntity<String> register(@RequestBody Account account) {
		...
		// try {
			...
			if (newDbAccount != null) {
				UserDetails newAccount = getUserDetailsWithEncodedPassword(newDbAccount);
				// inMemoryUserDetailsManager.createUser(newAccount);
				logger.info("{} was created", account.getUsername());

				status = HttpStatus.CREATED;
				body = Utility.fillResponseBody(status, AccountMessenger.ACCOUNT_CREATED, newAccount);
				// var location = ServletUriComponentsBuilder.fromUriString("/api/v1/auth/login/")
				// 		.toUri();
	...
	public ResponseEntity<String> authenticate(@RequestBody Account account) {
		...
		if (accountFromDb != null) {
			...
			if (passwordEncoder.matches(account.getPassword(),
				...
				body = Utility.fillResponseBody(status, AccountMessenger.ACCOUNT_FOUND, List.of(jwtToken));
			} else {
